...
- Антигаллюцинация: при ошибке API валют агент честно сообщает об ошибке
- Контейнеризация: `docker compose up` одной командой
- 11 unit-тестов (6 на `convert_currency`, 5 на `get_obligations`)
...
| Ограничение | Причина |
|---|---|
| **frankfurter.app НЕ поддерживает RUB.** Все запросы вида `convert_currency(amount, from, "RUB")` возвращают `ok=false`. | Это особенность источника данных: frankfurter.app использует курсы Европейского центрального банка (ECB Reference Rates). ECB прекратил публикацию курса RUB в марте 2022 года. **Это не баг агента, а объективное ограничение API.** Агент честно сообщает об этом пользователю и предлагает конвертацию в USD/EUR/GBP. Если нужен именно RUB — можно подключить дополнительный источник курса ЦБ РФ, но это расширение ТЗ. |
| **Курсы обновляются 1 раз в рабочий день.** Агент не увидит внутридневное изменение курса. | Источник данных — frankfurter.app, который использует курсы ECB Reference Rates. ECB публикует их раз в рабочий день около 16:00 CET. Это особенность источника, не агента. Параметр `force_refresh=True` позволяет сделать свежий запрос, если нужно перечитать. |
...
| **Нет ретраев при 429/5xx от frankfurter.app.** | ТЗ требует антигаллюцинацию (честная ошибка), а не экспоненциальный backoff. При необходимости добавляется одной декораторной обёрткой. |
...
| Минимум 3 unit-теста | 11 тестов (6 + 5) |
